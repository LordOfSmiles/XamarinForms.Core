...
namespace XamarinForms.Core.Extensions
{
    public static class XamarinFormsMarkupExtensions
    {
        ...
        public static Label Italic(this Label label)
        {
        ...
        public static TLabel FormattedText<TLabel>(this TLabel label, params Span[] spans) where TLabel : Label
        {
            ...
            foreach (var span in spans) label.FormattedText.Spans.Add(span);
            return label;
        }
        ...
        public static TView Font<TView>(this TView view, double? fontSize = null, bool? bold = null, bool? italic = null, string family = null) where TView : View
        {
            ...
            {
                case Button button:
                    if (fontSize.HasValue) button.FontSize = fontSize.Value;
                    if (attributes.HasValue) button.FontAttributes = attributes.Value;
                    if (family != null) button.FontFamily = family;
                    break;
                case Label label:
                    if (fontSize.HasValue) label.FontSize = fontSize.Value;
                    if (attributes.HasValue) label.FontAttributes = attributes.Value;
                    if (family != null) label.FontFamily = family;
                    break;
                case Entry entry:
                    if (fontSize.HasValue) entry.FontSize = fontSize.Value;
                    if (attributes.HasValue) entry.FontAttributes = attributes.Value;
                    if (family != null) entry.FontFamily = family;
                    break;
                case Picker picker:
                    if (fontSize.HasValue) picker.FontSize = fontSize.Value;
                    if (attributes.HasValue) picker.FontAttributes = attributes.Value;
                    if (family != null) picker.FontFamily = family;
                    break;
            }
    ...
    {
        public ToStringConverter(string format = "{0}") : base(o => string.Format(CultureInfo.InvariantCulture, format, o))
        {
        }
    }

    public class ShortTimeConverter : FuncConverter<DateTimeOffset>
    {
        public ShortTimeConverter() : base(t => t.ToString("t"))
        {
        }
    }
    ...
    public class Style<T> where T : Element // Should be IStyleElement but can't use that because it's internal
    {
        ...
        {
            foreach (var setter in setters) FormsStyle.Setters.Add(setter.Property, setter.Value);
            return this;
        }

        public Style<T> Add(params Behavior[] behaviors)
        {
            foreach (var behavior in behaviors) FormsStyle.Behaviors.Add(behavior);
            return this;
        }

        public Style<T> Add(params TriggerBase[] triggers)
        {
            foreach (var trigger in triggers) FormsStyle.Triggers.Add(trigger);
            return this;
        }
