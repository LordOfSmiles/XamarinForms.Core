...
namespace XamarinForms.Core.Droid.Renderers.ElegantTabs
{
    public sealed class ElegantTabRenderer : TabbedPageRenderer, BottomNavigationView.IOnNavigationItemSelectedListener
    {
            ...
            for (int i = 0; i < Element.Children.Count; i++)
            {
                ...
                if (i == selectedIndex)
                {
                    if (!Transforms.GetKeepIconColourIntact(page))
                    {
                        ...
                        {
                            menu.GetItem(i).SetIcon(IdFromTitle(page.IconImageSource, ResourceManager.DrawableClass));
                        }
                        else
                        {
        ...
        public new bool OnNavigationItemSelected(IMenuItem item)
        {
            ...
            if (!Transforms.GetDisableLoad(page))
            {
                if (_bottomNavigationView != null)
                {
                    if (_bottomNavigationView.SelectedItemId != item.ItemId && Element.Children.Count > selectedIndex 
                                                                            && selectedIndex >= 0)
                    {
